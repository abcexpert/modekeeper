...
def test_roi_before_after_savings_without_usd(tmp_path: Path, mk_path: Path) -> None:
    baseline_path = tmp_path / "baseline.jsonl"
    candidate_path = tmp_path / "candidate.jsonl"
    out_dir = tmp_path / "out"
    _write_step_time_jsonl(baseline_path, [100, 100, 100])
    ...
    summary = payload.get("summary", {})
    assert float(summary.get("gpu_hours_saved_per_hour")) > 0.0
    assert float(summary.get("gpu_hours_saved_per_month")) > 0.0
    assert summary.get("usd_per_gpu_hour") is None
    assert summary.get("usd_saved_per_gpu_hour") is None
    assert summary.get("usd_saved_per_hour") is None
    assert summary.get("usd_saved_per_month") is None
    assert "usd_per_gpu_hour_missing" in payload.get("notes", [])
...
def test_roi_before_after_help_is_wired(mk_path: Path) -> None:
    ...
    assert "USD cost per GPU-hour (optional)" in help_text
